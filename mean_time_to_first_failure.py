# -*- coding: utf-8 -*-
"""
Created on Wed Oct 17 10:11:34 2018

@author: Ehsan Hosseini
Firma: Rational AG
Projekt: Masterarbeit
Arbeitspacket: 01
Aufgaben: Mean Time to First Failure(MTFF) -> Average Time to First Service
...
def animate(df, field_name):
    Meldungen, Equipments, MTFF, df = func_MTFF_nach_X(df_filter_XY, field_name, failure_start_field_name=str('Störungsbeginn (Dat)_punkt'))
    f = Figure(figsize=(5,5), dpi=100)
    a = f.add_subplot(111)
...
def view_chart(caller,df, field_name):
    ...
    rects2 = ax.bar(ind+bar_width , count, bar_width, alpha=opacity, color='r', error_kw=error_config, label='number of Equipment')
...
class Rational(tk.Tk):
    def __init__(self, *args, **kwargs):    # * collects all the positional arguments in a tuple(list). ** collects all the keyword arguments in a dictionary.
        ...
        filemenu.add_command(label="save setting", command=lambda: self.action_save(title="Please select a file name for saving:", as_file=True))
        filemenu.add_separator()
        filemenu.add_command(label="import Excel", command= lambda: self.action_import_xlsx(title="Please select a file name to importing"))
        filemenu.add_separator()
        ...
        for F in (StartPage, Calender, PageOne, PageTwo, PageThree, PageFour, PageFive):
    ...
    def action_import_xlsx(self, title=None, file_name=None, dir_name=None, file_Ext=".xlsx", file_types=None, reset_index=True):
    ...
    def action_save(self, title=None, file_name=None, dir_name=None, file_Ext=".xlsx", file_types=None, as_file=False):
        ...
        if file_types is None:
            ...
            file_types = [('all files', '*.*'), ('text files', '*.txt'), ('python files','*.py'), ('excel files','*.xlsx'), ('json files', '*.JSON')]
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        combobox_land = ttk.Combobox(self, textvariable=self.v_land , values = ls_laender, postcommand=self.updtlistbox_ketten)
        combobox_land.pack()
        ...
        self.str_var_listbox_land = tk.StringVar(value=self.ls_selected_countries)    #option value=my_list ist optional no ncessary->initial values
        self.str_var_listbox_land.set(self.ls_selected_countries)            #StringVar str_var_listbox_land associated(fill) with str_var_listbox_land
        ...
        self.widget_set_position(button_reset, self.combobox_ketten, 200, False , 70, True)      # widget set position an Form
        ...
        self.str_var_listbox_ketten = tk.StringVar(value=self.ls_selected_ketten)    #option value=my_list ist optional->initial values
        ...
        self.str_var_listbox_ketten.set(self.ls_selected_ketten)       #StringVar str_var_listbox_ketten associated(fill) with str_var_listbox_ketten
        self.scrollbar.config(command=self.listbox_ketten.yview)
        self.listbox_ketten.pack()
        self.widget_set_position(button_reset, self.listbox_ketten, 200, False , 110, True)      # widget set position an Form
        ...
        self.combobox_geraete_große = ttk.Combobox(self, textvariable=self.v_geraete_große, values = self.ls_geraete_große, state='readonly')
        self.combobox_geraete_große.current(0)
        self.combobox_geraete_große.bind("<<ComboboxSelected>>", self.callback_geraete_große)
        self.widget_set_position(button_reset, self.combobox_geraete_große, 400, False , 70, True)      # widget set position an Form
        
        label_geraeten_großen = tk.Label(self, text="selected Device size")
        label_geraeten_großen.pack()
        self.widget_set_position(button_reset, label_geraeten_großen, 400, False , 90, True)      # widget set position an Form
        ...
        self.widget_set_position(button_reset, self.listbox_geraete_große, 400, False , 110, True)      # widget set position an Form
        ...
        self.combobox_baugruppe = ttk.Combobox(self, textvariable=self.v_baugruppe, values = self.ls_baugruppe, state='readonly') #    für die änderungen on combobox reicht es combobox.config(values=new values)
        self.combobox_baugruppe.current(0)
        self.combobox_baugruppe.bind("<<ComboboxSelected>>", self.callback_baugruppe)
        self.widget_set_position(button_reset, self.combobox_baugruppe, 600, False , 70, True)      # widget set position an Form
        ...
        self.str_var_listbox_baugruppe.set(self.ls_selected_baugruppe)  #   immer is set to ls_selected_baugruppe, bei änderungen in ls_selected_baugruppe wirkt automatisch on diese StringVar
        self.widget_set_position(button_reset, self.listbox_baugruppe, 600, False , 110, True)      # widget set position an Form
        ...
        self.widget_set_position(button_reset, self.entry_Equipments, 900, False , 90, True)      # widget set position an Form
        ...
        self.widget_set_position(button_reset, self.entry_Meldungen, 900, False , 130, True)      # widget set position an Form
        ...
        entry_zeitraum_fertigung_start = tk.Entry(self, textvariable=self.str_var_entry_zeitraum_fertigung_start, justify='center')
        ...
        entry_zeitraum_fertigung_ende = tk.Entry(self, textvariable=self.str_var_entry_zeitraum_fertigung_ende, justify='center')
        ...
        entry_zeitraum_meldung_ende = tk.Entry(self, textvariable=self.str_var_entry_zeitraum_meldung_ende, justify='center')
    ...
    def reset_func(self, ls_laender):
        ...
        self.updtlistbox_X(self.listbox_geraete_große, self.str_var_listbox_geraete_große, self.ls_selected_geraete_große)
    ...
    def callback_ketten(self,event):   #function called when '<<ComboboxSelected>>' event is triggered  ->click on element in combobox to add into listbox
    ...
    def func_filter_by_date(self, df, field_name ,str_var_entry_zeitraum_start, str_var_entry_zeitraum_ende):
        ...
        end_date = datetime.strptime(end_date_string, '%d.%m.%Y').date()    #string parser liefert datetime.date(year , month, 1)
    ...
    def call_func_filterXY(self):
        ...
        if ( self.str_var_entry_zeitraum_fertigung_start.get() != '' or self.str_var_entry_zeitraum_fertigung_ende.get() != '' ):
            #call filter by date -> truncate before and after eingegebene zeitraum
            df_filter_XY = self.func_filter_by_date(df_filter_XY, 'Fertigungsdatum', self.str_var_entry_zeitraum_fertigung_start, self.str_var_entry_zeitraum_fertigung_ende)
        ...
        if ( self.str_var_entry_zeitraum_meldung.get() != '' or self.str_var_entry_zeitraum_meldung_ende.get() != '' ):
            #call filter by date -> truncate before and after eingegebene zeitraum
            df_filter_XY = self.func_filter_by_date(df_filter_XY, 'Störungsbeginn (Dat)_punkt' , self.str_var_entry_zeitraum_meldung, self.str_var_entry_zeitraum_meldung_ende)
        ...
        ls_df_str_var_field_ls_sel =[ ( df_land_temp ,str_var_land ,'Land', ls_sel_countries) , (df_ketten_zeich_temp, str_var_ketten_zeich , 'ketten_zeich' , ls_sel_ketten) , (df_Geraete_große_temp, str_var_geraete_große , 'Geräte_große', ls_sel_geraete) ]  #   , (df_Baugruppe_temp, str_var_baugruppe , 'Baugruppe', ls_sel_baugruppe)
        #'Unnamed: 6' 
        empty_flag = True
        #index=0
        for df, str_var,field, ls_sel in ls_df_str_var_field_ls_sel:
            ...
            if ls_sel != [] :
                ...
                string_comb_var = "df is{}, str_var is {}, field is {}, ls_sel is {} ".format(df, str_var, field, ls_sel)
                print(" string combination df variables ls =  " , string_comb_var)
                df_filter_XY = func_filter_nach_XY(df_filter_XY , str(field), ls_sel)
                Meldungen, Equipments, MTFF, df = func_MTFF_nach_X(df_filter_XY, field_name=str(field), failure_start_field_name=str('Störungsbeginn (Dat)_punkt'))
        ...
        if empty_flag == True:
            ...
            self.func_assignment_dfs(ls_dfs) # if other class this method called, this must be StartPage.func_assignment_dfs(ls_dfs) writen
...
class PageOne(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button2.pack()
        button3 = ttk.Button(self, text= "View Bar Chart Land", command= lambda: self.view_chart(df_land_temp, field_name=str('Land')))
        button3.pack()
        button4 = ttk.Button(self, text= "View Line Chart Land ", command= lambda: StartPage.view_chart(self, df_land_temp, field_name=str('Land')))
        button4.pack()
        
        button5 = ttk.Button(self, text = "View Bar chart func view_chart", command= lambda: view_chart(self,df_land_temp, field_name=str('Land')) )
        button5.pack()
...
class PageTwo(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button3 = ttk.Button(self, text= "View Line Chart Baugruppe", command= lambda: StartPage.view_chart(self, df_Baugruppe_temp, field_name=str('Baugruppe')))
...
class PageThree(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label.pack(side='top')
        button1 = ttk.Button(self, text="Back to Home", command=lambda: self.back_home(controller, self.canvas, self.toolbar))
        button1.pack()
        button2 = ttk.Button(self, text= "Page Two nach Baugruppe", command=lambda: controller.show_frame(PageTwo))
        button2.pack()
        button3 = ttk.Button(self, text= "View Line Chart ketten_zeich", command= lambda: StartPage.view_chart(self, df_ketten_zeich_temp, field_name=str('ketten_zeich')))
        button3.pack()
...
class PageFour(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label.pack(side = 'top')
        button1 = ttk.Button(self, text="Back to Home", command=lambda: PageThree.back_home(self, controller, self.canvas, self.toolbar))
        button1.pack()
        button2 = ttk.Button(self, text= "Page Three nach Ketten_zeichen", command=lambda: controller.show_frame(PageThree))
        button2.pack()
        button3 = ttk.Button(self, text= "View Line Chart Geraete_große", command= lambda: StartPage.view_chart(self, df_Geraete_große_temp, field_name=str('Geräte_große')))
        button3.pack()
...
class PageFive(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button2.pack()
        button3 = ttk.Button(self, text= "View Bar chart Geräte_geroße animation", command=lambda: self.call_animation(df_Geraete_große_temp, field_name=str('Geräte_große')))
        button3.pack()
...
file_name_Punktediagramm = 'Punktediagramm 10_2018 inkl. RSP.xlsx'
...
def func_prepare_data(data_path, file_name_Punktediagramm, sheet_name):
    ...
    df_main_data = df_main_data.reset_index(inplace= True, drop=True)  # reset indexes after vertical join to avoid same indexes
...
def func_join_dfs(df_1, df_2, df_3):
    ...
    df = df.set_index('Service-Meldung').join(df_temp.set_index('Service-Meldung'), on='Service-Meldung', how='left', rsuffix='_')
    ...
    df = df.fillna(0)   # remove null values after join with df_Meldungen because of records which there is not in df_Meldungen
    ...
    #   Aggregate
    df = df.groupby(['Service-Meldung', 'Equipment', 'Src_meldung_Ursachencode', 'Baugruppe', 'Bau_Gr_kennz', 'Land', 'Germat Elektro/Gas (', 'ketten_zeich', 'mat_Spannung', 'Anw-Status', 'Invt_Nr', 'Fertigungsdatum', 'GewährlBeginn Datum', 'Störungsbeginn (Dat)_punkt' , 'Störungsbeginn (Dat)', 'Meldungsart', 'Meldungsdatum', 'StörBeginn', 'Störungsende' ]).size().reset_index(name='count')
...
def func_join_to_df(*dfs, **fields):
    ...
    df = df_caller.join(df_temp.set_index('Service-Meldung'), on='Service-Meldung', how='left', rsuffix='_K') #  join df_caller and df_temp
    ...
    df = df_caller.join(df_other.set_index('Service-Meldung'), on='Service-Meldung', how='left', rsuffix='_K') #  join df_caller and df_kundeninfo
    df = df.fillna(0) # remove NaN values
...
def func_maintain_df(df_1, df_2):
    ...
    df_Meldung_Beginn_Ende['Meldungsdatum']    = [datetime.strftime( x , "%d.%m.%Y" ) if not pd.isnull(x) else '' for x in  df_Meldung_Beginn_Ende['Meldungsdatum']    ]
    df_Meldung_Beginn_Ende['StörBeginn']       = [datetime.strftime( x , "%d.%m.%Y" ) if not pd.isnull(x) else '' for x in  df_Meldung_Beginn_Ende['StörBeginn']       ]
    df_Meldung_Beginn_Ende['Störungsende']     = [datetime.strftime( x , "%d.%m.%Y" ) if not pd.isnull(x) else '' for x in  df_Meldung_Beginn_Ende['Störungsende']     ]
...
Vars_func_MTFF={}
def func_MTFF(df, field_name):      #field_name determiend the field of Start of Stoerung, because of df_temp hier field_name must be 'Störungsbeginn (Dat)_punkt'
    ...
    for i in range (len(df)):
        if all(v==np.empty for v in df.iloc[i].values) is False:
                ls_F_Failure_time.append(abs( (datetime.strptime( df[fieldName][i], "%d.%m.%Y"  ) - datetime.strptime(df['GewährlBeginn Datum'][i], "%d.%m.%Y") ).days ) )
        else:
    ...
    number_of_effective_Meldungen = len(df) - len(df[ (df['F_t_Fail_time_days']==0) ].count())     # Anzahl der Equipment ohne leere Meldungsdatum
    MTFF = df['F_t_Fail_time_days'].sum() // number_of_effective_Meldungen      # calculate the Average -> Mean Time to first Failure
...
def func_MTFF_nach_Land(df, field_name):        #field_name determiend the field of Start of Stoerung, because of df_temp hier field_name must be 'Störungsbeginn (Dat)_punkt'
    ...
    df_MTFF_nach_Land = df_MTFF_nach_Land.groupby(['Service-Meldung', 'Equipment', 'Land', 'GewährlBeginn Datum', fieldName]).size().reset_index(name='count')
    
    df_MTFF_nach_Land = df_MTFF_nach_Land.drop(columns='count')
    
    df_MTFF_nach_Land = df_MTFF_nach_Land.groupby(['Equipment', 'Land', 'GewährlBeginn Datum', fieldName]).size().reset_index(name='count')
    ...
    for i in range (len(df_MTFF_nach_Land)):
        if all(v==np.empty for v in df_MTFF_nach_Land.iloc[i].values) is False:
               ls_F_Failure_time.append(abs( (datetime.strptime( df_MTFF_nach_Land[fieldName][i], "%d.%m.%Y"  ) - datetime.strptime(df_MTFF_nach_Land['GewährlBeginn Datum'][i], "%d.%m.%Y") ).days ) )
        else:
            ls_F_Failure_time.append(0)
    
    df_MTFF_nach_Land = df_MTFF_nach_Land.assign(F_t_Fail_time_days = ls_F_Failure_time ) # create new column "F_t_Fail_time_days"
    
    number_of_effective_Meldungen = len(df_MTFF_nach_Land) - len(df_MTFF_nach_Land[ (df_MTFF_nach_Land['F_t_Fail_time_days']==0) ].count(1))
    ...
    df_MTFF_nach_Land = df_MTFF_nach_Land[(df_MTFF_nach_Land['F_t_Fail_time_days'] != 0)]   # remove rows from df in which one of the columns has a value of 0
    
    df_MTFF_nach_Land = df_MTFF_nach_Land.drop(columns='count')
    
    df_MTFF_nach_Land = df_MTFF_nach_Land.groupby( ['Land', 'F_t_Fail_time_days'] ).size().reset_index(name='count')
    
    df_MTFF_nach_Land = df_MTFF_nach_Land.groupby( ['Land'] ).agg({'F_t_Fail_time_days': {'mean_failure_time':'mean'} , 'count':{'sum_count' : 'sum'} })
    ...
    write_to_excel(df_MTFF_nach_Land, data_path, file_name='MTFF_nach_Land.xlsx', sheet_name='2009-'+str(current_year))    
...
def func_MTFF_nach_X(df_X, field_name, failure_start_field_name):
    ...
    df_X = df_X.groupby(['Service-Meldung', 'Equipment', field, 'GewährlBeginn Datum', failureStart]).size().reset_index(name='count')
    ...
    for i in range (len(df_X)):
        if all(v==np.empty for v in df_X.iloc[i].values) is False:
               ls_F_Failure_time.append(abs( (datetime.strptime( df_X[failureStart][i], "%d.%m.%Y"  ) - datetime.strptime(df_X['GewährlBeginn Datum'][i], "%d.%m.%Y") ).days ) )
        else:
            ls_F_Failure_time.append(0)
    ...
    MTFF = df_X['F_t_Fail_time_days'].sum() // number_of_effective_Meldungen      # calculate the Average -> Mean Time to first Failure
    print("Mean Time to First Failure: %.3f days" % MTFF)    #df_test = df_test.dropna(how='any', axis=0)       # remove all rows from df in which one of the columns has a damy of value -> np.NaN
    ...
    df_X = df_X.groupby( [field] ).agg({'F_t_Fail_time_days': {'mean_failure_time':'mean'} , 'count':{'sum_count' : 'sum'} })
...
class Feature_selection(object):
    ...
    def prepare_df(self, index_item):
        ...
        if index_item == 2 :
            df = df.groupby(['sorted_Störungsbegin','Failure_time']).size().reset_index(name='count')#    	mean of sum of the Equipment
    ...
    def plot_Outliers(self, df):
        ...
        for name, group in annual_groups:
            ...
            if len(group.values) is 12:
                ...
                years[name.year] = years[name.year].str.get(0) #    convert each columns value to value remove square bracket [] from values
...
def main():
    ...
    '''     Archive read from Excel(use slowly method load excel) and write df to JSON
    
    #prepare DataFrames from file_name:'Punktediagramm 10_2018 inkl. RSP.xlsx', merge sheet names 2009 bis current year
    ##sheet_name = list(range(2009,2018+1)) # in range 2009 to include 2018 (dynamic current year)
    df_punktediagram = func_prepare_data(data_path, file_name_Punktediagramm, sheet_name)
    #         reset indexes is in func_prepare_data
    #reset indexes before write to JSON is necessary
    ##df_punktediagram.reset_index(inplace= True, drop=True)  # reset indexes after vertical join to avoid same indexes
    
    func_write_df_to_JSON(df_punktediagram, file_name_Punktediagramm) # Write dataframes to JSON

    #Prepare Dataframes from file_name: 'SAP_Meldungsnummer_Installationsdatum_Customer.xlsx', merge sheet_names = 2009 bis current year
    df_installationsdatum_customer = func_prepare_data(data_path, file_name_Installationsdatum_customer, sheet_name)
    #         reset indexes is in func_prepare_data
    #reset indexes before write to JSON is necessary
    ##df_installationsdatum_customer.reset_index(inplace=True, drop=True)   # reset indexes after vertical join to avoid same indexes
    
    func_write_df_to_JSON(df_installationsdatum_customer, file_name_Installationsdatum_customer)   # Write dataframes to JSON

    #Prepare Dataframes from file_name: 'Meldungsnummer Beginn Ende.xlsx', merge sheet_names = 2009 bis current year
    df_Meldung_Beginn_Ende = func_prepare_data(data_path, file_name_Meldung_Beginn_Ende, sheet_name)
    #         reset indexes is in func_prepare_data
    #reset indexes before write to JSON is necessary
    ##df_Meldung_Beginn_Ende.reset_index(inplace=True, drop=True)   # reset indexes after vertical join to avoid same indexes
    
    func_write_df_to_JSON(df_Meldung_Beginn_Ende, file_name_Meldung_Beginn_Ende)   # Write dataframes to JSON
    
    '''     # Ende Archive  read from Excel(slowly method) and write df to JSON
    
    
    
    
    
    '''     Archive read from Excel(use fast method load excel) and write df to JSON

    '''     # Ende Archive  read from Excel(fast method) and write df to JSON
    
    ''' #22.11.2018
    df_punktediagram                    = load_excelFile_all_sheets(file_name_Punktediagramm, reset_index=True)   # read from excel method_2
    func_write_df_to_JSON(df_punktediagram, file_name_Punktediagramm) # Write dataframes to JSON
    df_installationsdatum_customer      = load_excelFile_all_sheets(file_name_Installationsdatum_customer, reset_index=True)  # read from excel method_2
    func_write_df_to_JSON(df_installationsdatum_customer, file_name_Installationsdatum_customer)   # Write dataframes to JSON
    df_Meldung_Beginn_Ende              = load_excelFile_all_sheets(file_name_Meldung_Beginn_Ende, reset_index=True)  # read from excel method_2
    func_write_df_to_JSON(df_Meldung_Beginn_Ende, file_name_Meldung_Beginn_Ende)   # Write dataframes to JSON
    df_kundeninfo = load_excelFile_all_sheets(file_name_kundeninfo, reset_index=True)
    func_write_df_to_JSON(df_kundeninfo, file_name_kundeninfo)


    
    #   read dataframes from JSON files
    df_punktediagram_json = func_read_df_from_json(file_name_Punktediagramm)
    df_installationsdatum_customer_json = func_read_df_from_json(file_name_Installationsdatum_customer)
    df_Meldung_Beginn_Ende_json = func_read_df_from_json(file_name_Meldung_Beginn_Ende)     # Equipment ab sheet 2011 während der laden von Excel Datei hat sich geändert und in JSON Datei auch die Datum
    df_kundeninfo = func_read_df_from_json(file_name_kundeninfo)
    
    #       manipulate df such as falsch values
    #       df_Meldung_Beginn_Ende_json Dates correction and gleich convert to standard format "%d.%m.%Y"
    df_Meldung_Beginn_Ende, df_Meldung_Beginn_Ende_json = func_maintain_df(df_Meldung_Beginn_Ende, df_Meldung_Beginn_Ende_json)
    
    #   write df to JSON after write corrected Values
    func_write_df_to_JSON(df_Meldung_Beginn_Ende_json, file_name_Meldung_Beginn_Ende)
    #22.11.2018 '''
    
    
    
    '''
    # Arbeiten mit data dictionary
    df_2 = df_installationsdatum_customer_json.set_index('Service-Meldung').to_dict()
    df_2.get('Service-Meldung', 347676)     # output -> 347676
    df_2['Equipment'].get(336342)   # output-> 2168015  für ein Key(336342=Service-Meldung) in Equipment liefert Equipment Nr
    for Equipment in df_2:
        Equipment = df_2['Equipment']
        i =len(Equipment)
    print(i)    # 299862 lenght of df_2['Equipment']  
    len(df_2)   # 10 lenght of df_2
    len(df_2['Equipment'])
    for key, value in sorted(df_2.items()):
        print(key)  #output-> aller Key
    for Equipment in sorted(df_2, key=len):
        print(Equipment, df_2[Equipment])   # print all Keys and Values
    #Counting
    count={}
    for element in df_2:
        count[element]=count.get(element,0)+1
    print(count)    # output-> aller Key und count ist eine neue data dict aber ohne values
    ...
    df = func_join_dfs(df_punktediagram_json, df_installationsdatum_customer_json, df_Meldung_Beginn_Ende_json)
    fields = { 'f3':'Kettenkunde', 'f4':'Unnamed: 6', 'f5':'Kunde', 'f6':'Unnamed: 8' , 'f7':'RSP-Nummer'} #    df_kundeninfo columns
    #   join df_kundeninfo to df
    df = func_join_to_df(*[df, df_kundeninfo], **fields)
    df_test = df[ (df['Equipment']==2288782) ]      # test join is right
    #   write df result of join of 3 df to excel and to json
    write_to_excel(df, data_path, file_name_joint_df, sheet_name='2009-'+str(current_year))
    func_write_df_to_JSON(df, file_name_joint_df)
    #22.11.2018'''
    ...
    df = func_read_df_from_json(file_name_joint_df)
    ...
    ''' # 04.12.2018
    df_sorted[ (df_sorted['Equipment']==2168522) ]
    df_temp=pd.DataFrame()      # create temp df
    df_temp = df_sorted.groupby(['Service-Meldung', 'Equipment', 'GewährlBeginn Datum', fieldName]).size().reset_index(name= 'count')    # groupby in order to get 4 fields
    df_temp = df_temp.drop(columns='count')
    df_temp = df_sorted.groupby(['Equipment', 'GewährlBeginn Datum', fieldName]).size().reset_index(name= 'count')    # groupby in order to get 3 fields
    df_temp[ (df_temp['Equipment']==2168522) ]
    df_temp = df_temp.sort_values(['Equipment', fieldName], ascending=True)      # sorting values by the 'Störungsbeginn (Dat)_punkt'
    
    df_temp['sorted_Date'] = pd.to_datetime(df_temp[fieldName])
    df_temp = df_temp.sort_values([ 'Equipment' , 'sorted_Date'], ascending=True)
    
    df_temp = df_temp.drop_duplicates('Equipment',keep='first')     # remove duplicated Equipments Nr in order to remove duplicated Meldungsdatum -> (field_name)
    df_temp = df_temp.drop(columns='sorted_Date')
    ...
    #MTFF, df_MTFF = func_MTFF(df_temp, field_name = str('Störungsbeginn (Dat)_punkt'))      #field_name determiend the field of Start of Stoerung
    #print("Mean Time to First Failure: %.3f days" % MTFF)
    
    
    
    #df_MTFF_nach_Land = df_sorted.drop(columns='count')
    #func_MTFF_nach_Land(df_MTFF_nach_Land, field_name = str('Störungsbeginn (Dat)_punkt'))
    
    
    
    #   MTFF nach Baugruppe
    global df_Baugruppe
    Meldungen, Equipments, MTFF, df_Baugruppe = func_MTFF_nach_X(df_sorted, field_name=str('Baugruppe'), failure_start_field_name=str('Störungsbeginn (Dat)_punkt'))
    #   MTFF nach ketten_zeich
    global df_ketten_zeich
    Meldungen, Equipments, MTFF, df_ketten_zeich = func_MTFF_nach_X(df_sorted, field_name=str('ketten_zeich'), failure_start_field_name=str('Störungsbeginn (Dat)_punkt'))
    #   MTFF nach land
    global df_land
    Meldungen, Equipments, MTFF, df_land = func_MTFF_nach_X(df_sorted, field_name=str('Land'), failure_start_field_name=str('Störungsbeginn (Dat)_punkt'))
    #   MTFF nach Inventar Nr.
    ##func_MTFF_nach_X(df_sorted, field_name=str('Invt_Nr'))
    global df_Geraete_große
    df_sorted['Geräte_große'] = df_sorted['Invt_Nr'].str[1] + df_sorted['Invt_Nr'].str[2]   # create device size field
    Meldungen, Equipments, MTFF, df_Geraete_große = func_MTFF_nach_X(df_sorted, field_name = str('Geräte_große'), failure_start_field_name=str('Störungsbeginn (Dat)_punkt'))
    
    #   Save mentioned above dfs into JSON to read from JSON at each run, thereby compiling faster
    func_write_df_to_JSON(df_land, 'df_land' )
    func_write_df_to_JSON(df_ketten_zeich, 'df_ketten_zeich')
    func_write_df_to_JSON(df_Geraete_große, 'df_Geraete_große')
    func_write_df_to_JSON(df_Baugruppe, 'df_Baugruppe')
    
    
    #   Save mentioned above dfs into xlsx to read from xlsx at each run, thereby compiling faster
    write_to_excel(df_land, data_path, file_name='df_land.xlsx', sheet_name = '2009-'+str(current_year) )
    write_to_excel(df_ketten_zeich, data_path, file_name='df_ketten_zeich.xlsx', sheet_name = '2009-'+str(current_year) )
    write_to_excel(df_Geraete_große, data_path, file_name='df_Geraete_große.xlsx', sheet_name = '2009-'+str(current_year) )
    write_to_excel(df_Baugruppe, data_path, file_name='df_Baugruppe.xlsx', sheet_name = '2009-'+str(current_year) )
    
    #   machine learning
    feature_sel = Feature_selection(df_sorted)
    df_Equipment = feature_sel.prepare_df(1)
    df_Stoerungen = Feature_selection.prepare_df(feature_sel, 2)
    #   save df Equipment
    func_write_df_to_JSON(df_Equipment, 'df_Equipment')
    write_to_excel(df_Equipment, data_path, 'df_Equipment.xlsx', 'Time_to_Failure')
    #save df Stoerungen
    func_write_df_to_JSON(df_Stoerungen, 'df_Stoerungen')
    write_to_excel(df_Stoerungen, data_path, 'df_Stoerungen.xlsx', 'Time_to_Failure')
    
    #27.11.2018'''
    
    #   read mentioned above dfs from JSON faster than each time to compiling
    ##df_land = func_read_df_from_json('df_land')                       #funktioniert nicht
    ##df_ketten_zeich = func_read_df_from_json('df_ketten_zeich')       #funktioniert nicht
    ##df_Geraete_große = func_read_df_from_json('df_Geraete_große')     #funktioniert nicht
    ##df_Baugruppe = func_read_df_from_json('df_Baugruppe')             #funktioniert nicht
    
    print("fourth timer", datetime.now() ) # fourth timer
    
    global df_land, df_ketten_zeich, df_Geraete_große ,df_Baugruppe
    #   read mentioned above dfs from xlsx faster than each time to compiling
    df_land             = load_excelFile_all_sheets('df_land.xlsx', reset_index=False)   # read from excel method_2 faster method
    df_ketten_zeich     = load_excelFile_all_sheets('df_ketten_zeich.xlsx', reset_index=False)   # read from excel method_2 faster method
    df_Geraete_große    = load_excelFile_all_sheets('df_Geraete_große.xlsx', reset_index=False)   # read from excel method_2 faster method
    df_Baugruppe        = load_excelFile_all_sheets('df_Baugruppe.xlsx', reset_index=False)   # read from excel method_2 faster method
    ...
    collection = []
    dateTimeString = datetime.today() # or str(datetime.today())
    collection.append(dateTimeString)
    print(collection)
    #  Ende Archive usefull code '''
    
    
    '''
    #initialize df_temp_s is already declarated in class StartPage
    global df_land_temp
    df_land_temp = df_land.copy(deep=True)
    global df_ketten_zeich_temp
    df_ketten_zeich_temp = df_ketten_zeich.copy(deep=True)
    ...
    # read JSON Service.txt Data, Service_4.txt -> Service_9.JSON
    df_services = pd.read_json('Service_9.JSON')
    df_services = df_services.fillna(0)
    df_Stoerungen = func_read_df_from_json('df_Stoerungen')
    df_Equipment = func_read_df_from_json('df_Equipment')
    
    '''
    # machine learing runing
    feature_sel = Feature_selection(df_sorted) #    create a object of Class
    Feature_selection.arima(feature_sel, df_Stoerungen) #   call function Class
    # '''
